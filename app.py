...
st.markdown("""
    <style>
        ...
        .stTabs [data-baseweb="tab"] {
            ...
            box-shadow: 0px 4px 12px rgba(112, 144, 176, 0.05);
        }
...
# [마스터 기간] - 2026년 1월 ~ 12월 자동 생성 (데이터 없어도 표시)
current_year = datetime.now().year
# 혹시 시트에 2025, 2027 등이 있을 수 있으니 감안하되 기본 1~12월 생성
full_months = [f"{current_year}-{str(m).zfill(2)}" for m in range(1, 13)]
master_months = ["전체 누적"] + sorted(full_months, reverse=True)
...
# =============================================================================
    ...
    with st.sidebar:
        st.subheader("Filter")
        # [수정] 마스터 팀 목록 사용
        leave_dept_option = st.selectbox("소속 부서", master_teams)
...
# =============================================================================
    ...
    # [수정] 고급 탭 UI (st.tabs 사용)
    tab_monthly, tab_weekly = st.tabs(["📊 통합 현황 (Monthly)", "📈 주간 추이 (Weekly)"])
    ...
    with tab_monthly:
        ...
        # Shiftee Style KPI Cards (상단 컬러바)
        k1, k2, k3, k4 = st.columns(4)
        with k1:
        ...
        with c1:
            ...
            # [수정] 차트용 팀 목록 확보 (데이터 없어도 0 표시)
            if ot_team_opt == "전체 팀":
                chart_teams = [t for t in master_teams if t != "전체 팀"]
            else:
